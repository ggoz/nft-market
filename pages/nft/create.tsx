...
import { useWeb3 } from "@/components/provides/web3";
...
const Create: NextPage = () => {
  ...
  const getSignedData = async () => {
    ...
    const signData = await ethereum?.request({
      ...
      params: [JSON.stringify(data.message), account, data.message.id]
    });
  ...
  const handleChange = (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    setNftMeta((state) => ({ ...state, [name]: value }));
  };

  // attack healthy speed 变化
  const handleAttributeChange = (e: ChangeEvent<HTMLInputElement>) => {
    const { name, value } = e.target;
    const attributeIdx = nftMeta.attributes.findIndex((attribute) => attribute.trait_type === name);
  ...
  const handleImage = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const { account, signData } = await getSignedData();
      ...
      setNftMeta({ ...nftMeta, image: `${process.env.NEXT_PUBLIC_PINATA_DOMAIN}/ipfs/${data.IpfsHash}` });
    } catch (e: any) {
  ...
  const uploadMetadata = async () => {
    try {
      const { signData, account } = await getSignedData();
      ...
      const res = await toast.promise(promise, {
        ...
        success: "Upload metadata success",
  ...
  const createNft = async () => {
    try {
      ...
      const content = nftRes.data as PinataRes;
                ...
                <div className='shadow sm:rounded-md sm:overflow-hidden'>
                  ...
                  { nftURI && (
                    <div className='mb-4 p-4'>
                      ...
                      <div>
                        <Link href={ nftURI }>
                          <span className='underline text-indigo-600'>{ nftURI }</span>
                        </Link>
                      </div>
                    </div>
                  ) }
